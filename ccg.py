...
class Item:
    ...
    def signature(self):
        return ('I', self.category, self.i, self.j)

    def __repr__(self):
        return f"Item({self.category}, {self.i}, {self.j})"
    
    def __eq__(self, other):
        return isinstance(other, Item) and self.category == other.category and self.i == other.i and self.j == other.j

    def __hash__(self):
        return hash((self.category, self.i, self.j))
...
def is_axiom(item: Item) -> bool:
    return item.j == item.i + 1  # lexical items are [X; i; i+1]
...
def cky_backward(left: Item, right: Item) -> Union[None, Item]:
    ...
    :param right: right item in the form [X\\Y, i, j]
    :param left: left item in the form [Yβ, j, k]
    :return: new item in the form [Xβ, i, k] or None if not applicable.
    """
    category = right.category  
    parts = {category[:m.start()]: category[m.start()+1:]
             for m in re.finditer(r'\\([^/\\]+)', category)}
    for func_type, expected_arg in parts.items():
        left_category = left.category
        if (left_category == expected_arg or
            (left_category.startswith(expected_arg) and
             len(left_category) > len(expected_arg) and
             left_category[len(expected_arg)] in {'/', '\\'})):
            beta = left_category[len(expected_arg):]
            new_category = f"{func_type}{beta}" if beta else func_type
            return Item(new_category, left.i, right.j)
    return None
